...
data = read_xlsx('/Users/alishamsaley/Documents/GitHub/TF_CP3/FullData.xlsx')
...
#aggregating by pH to see ave time spent out of the water (pooled individuals)
d1agg = aggregate(d1$P_out2, by = list(d1$pH, d1$cue), FUN = mean)
plot(d1agg$x ~ d1agg$Group.1, col = as.factor(d1agg$Group.2), pch = 19)
```
plotting how many individuals leave water
```{r}
#for each pH level, calculate the number of individuals that left the water as a proportion to the total exposed to that cue type. 
d1$P_out3 = ceiling(d1$P_out2)  #ceiling will round up to the nearest integer; giving a vector of 0s and 1s..floor will round down.
dagg = aggregate(d1$P_out3, by = list(d1$pH, d1$cue), FUN = mean)
...
```{r}
par(mfrow = c(2,2))
with(d2, plot(P_bot ~ pH, type = "n", xlab = "pH acclimation", ylab = "prop time on bottom", bty = "n", main = "Prop bottom"))
...
#model to show behavior bottom

bott = glmer(cbind(Bottom, Not_bot) ~ cue * pH + (1|ind) + (1|accbin) + (1|arena) + (1|trial), data = d2, family = binomial, control= glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))
summary(bott)
#arena and trial were negli so dropped them from the model
#AIC 
trend1 = emtrends(wall, ~cue, var = "pH")
summary(trend1)
...
d1.1 = expand.grid(pH = seq(from = 6.1, to = 8.3, by = 0.05), cue = c('crab', 'no cue', 'pisaster'), Bottom = 0, Not_bot = 0)
mm1 = model.matrix(terms(bott), d1.1)
d1.1$out = mm1%*%fixef(bott)
#95CI around models; error bars
pvar1 = diag(mm1%*%tcrossprod(vcov(bott), mm1))
cmult = 1.96
newdat1 = data.frame(d1.1, plo = d1.1$out - cmult * sqrt(pvar1), phi = d1.1$out + cmult * sqrt(pvar1))

plot(plogis(out) ~ pH, ylim = c(0,1), xlim = c(6.1,8.3), data = d1.1, bty = "n", type = "n", ylab = "Proportion of time on the bottom of tank d2", xlab = "pH (Total scale)")
...
points(P_wall ~ pH, data = d2[d2$cue == "no cue",], col = "black", pch = 19)
points(P_wall ~ pH, data = d2[d2$cue == "pisaster",], col = "purple", pch = 19)
points(P_wall ~ pH, data = d2[d2$cue == "crab",], col = "red", pch = 19)

#model to show behavior wall
###model for time spent in corner.
wall = glmer(cbind(Wall, Not_wall) ~ cue * pH + (1|ind) + (1|accbin) + (1|arena) + (1|trial), data = d2, family = binomial, control= glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))
summary(wall)
#arena and trial were negli so dropped them from the model
#AIC 
trend1 = emtrends(wall, ~cue, var = "pH")
...
#prediction values from the glmer
d1.1 = expand.grid(pH = seq(from = 6.1, to = 8.3, by = 0.05), cue = c('crab', 'no cue', 'pisaster'), Wall = 0, Not_wall = 0)
mm1 = model.matrix(terms(wall), d1.1)
d1.1$out = mm1%*%fixef(wall)
#95CI around models; error bars
pvar1 = diag(mm1%*%tcrossprod(vcov(wall), mm1))
cmult = 1.96
newdat1 = data.frame(d1.1, plo = d1.1$out - cmult * sqrt(pvar1), phi = d1.1$out + cmult * sqrt(pvar1))

plot(plogis(out) ~ pH, ylim = c(0,1), xlim = c(6.1,8.3), data = d1.1, bty = "n", type = "n", ylab = "Proportion of time on the wall d2", xlab = "pH (Total scale)")
CrCI = newdat1[newdat1$cue == "crab",]
PCI = newdat1[newdat1$cue == "pisaster",]
NoCI = newdat1[newdat1$cue == "no cue",]
...
```{r}
par(mfrow = c(2,2))
crack = d1[d1$cue == "crab" & d1$P_out2 < .25,]
View(crack)
...
points(P_cor ~ pH, data = crack, col = "black", pch = 19)


points(P_cor ~ pH, data = d2[d2$cue == "no cue",], col = "black", pch = 19)
points(P_cor ~ pH, data = d2[d2$cue == "pisaster",], col = "purple", pch = 19)
points(P_cor ~ pH, data = d2[d2$cue == "crab",], col = "red", pch = 19)


###model for time spent in corner.
cor = glmer(cbind(Corner, Not_cor) ~ pH + (1|ind) + (1|accbin) + (1|trial), data = crack, family = binomial, control= glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))
summary(cor)
#arena and trial were negli so dropped them from the model
#AIC 
...
plot(plogis(out) ~ pH, ylim = c(0,1), xlim = c(6.1,8.3), data = d1.1, bty = "n", type = "n", ylab = "Proportion of time in the corner d2", xlab = "pH (Total scale)")
CrCI = newdat1[newdat1$cue == "crab",]
PCI = newdat1[newdat1$cue == "pisaster",]
NoCI = newdat1[newdat1$cue == "no cue",]
...
with(d1, plot(P_bot ~ pH, type = "n", xlab = "pH", ylab = "prop time spending", bty = "n", main = "crab"))
points(P_wall ~ pH, data = d1[d1$cue == "crab",], col = "dark green", pch = 19)
points(P_cor ~ pH, data = d1[d1$cue == "crab",], col = "blue", pch = 19)
points(P_bot ~ pH, data = d1[d1$cue == "crab",], col = "black", pch = 19)

with(d1, plot(P_bot ~ pH, type = "n", xlab = "pH", ylab = "prop time spending", bty = "n", main = "pisaster"))
points(P_wall ~ pH, data = d1[d1$cue == "pisaster",], col = "dark green", pch = 19)
points(P_cor ~ pH, data = d1[d1$cue == "pisaster",], col = "blue", pch = 19)
points(P_bot ~ pH, data = d1[d1$cue == "pisaster",], col = "black", pch = 19)
...
plot(glmer1) # plots 4 graphs
qqnorm(resid(glmer1)) # check normality of variance
qqline(resid(glmer1)) # draw line on qq
hist(resid(glmer1)) # another way to check normality

# plot residuals against all predictors
# try to identify source of var het
plot(resid(glmer1) ~ d1$pH) #spread is low at ph 6.5 but but spreads out more by 6.8.
plot(resid(glmer1) ~ d1$cue) #much less variation at no cue treatment (potential driven by lower sampling size)
...
```{r}
par(mfrow = c(1,2))
plot(plogis(out) ~ pH, ylim = c(0,1), xlim = c(6.0,8.0), data = d1.1, bty = "n", type = "n", ylab = "Proportion of time out of water", xlab = "pH (Total scale)")
CrCI = newdat1[newdat1$cue == "crab",]
PCI = newdat1[newdat1$cue == "pisaster",]
NoCI = newdat1[newdat1$cue == "no cue",]
...
points(plogis(out) ~ pH, data = d1.1[d1.1$cue == 'crab',], col = "red", pch = 16)
points(plogis(out) ~ pH, data = d1.1[d1.1$cue == 'no cue',], col = "black", pch = 16)
#figuring out the decrease in ave prop out based on the model at a give pH value for a give cue type. this is not from bootstrap but will work for the mathias reporting.
View(d1.1)
d1.1$out_logtrans = plogis(d1.1$out)
plot(d1.1$out_logtrans ~ d1.1$pH, col = as.factor(d1.1$cue))
...
abline(v = 8, col = "blue", lwd = 2, lty = 5)
abline(v = 7.7, col = "orange", lwd = 2, lty = 5)
legend(x = 6.5, y = 1, legend = c('sea star', 'crab', 'no cue'), box.lty = 0, fill = c('purple', 'red', 'black'), bg = "white")
```
```{r}
...
```
####Now remove all of the individuals that left the water at all; this will remove any individuals that fled through the corner on their way "out of the water";.
```{r}
...
data3 = data%>% group_by(ind) %>% dplyr::summarise(pH = mean(pH_ave), accT = mean(acc_tank), Cue = mean(treatment), avX = mean(FX), avY = mean(FY), mdist = max(dist_filt), midist = min(dist_filt), avedis2 = mean(dist_filt), avedis = sqrt((avX^2) + (avY^2)), Group = mean (Trial_main), avdiscent = (sum(sqrt((FX-avX)^2 + (FY-avY)^2)))/(length(ind)), wand = avdiscent/(sqrt(avX^2 + avY^2)), total = sum(dist_filt),P_in1 = sum(in_out)/n(), P_in2 = sum(in_out2)/n(), In1 = sum(in_out), In2 = sum(in_out2), Total = length(ind), Out1 = Total - In1, Out2 = Total - In2, P_out1 = 1-P_in1, P_out2 = 1-P_in2, Corner = sum(corner), Not_cor = Total - Corner, Wall = sum(wall), Not_wall = Total - Wall, Bottom = sum(bottom), Not_bot = Total - Bottom, P_cor = sum(corner)/Total, P_wall = sum(wall)/Total, P_bot = sum(Bottom)/Total)
...
n=10
PrbPal <- colorRampPalette(c("black","red","orchid4"))
CrbPal <- colorRampPalette(c("black","red","orchid4"))
NrbPal <- colorRampPalette(c("black","red","orchid4"))
pis$p = PrbPal(n)[as.numeric(cut(pis$pH_ave,breaks = n))]
cra$cra = CrbPal(n)[as.numeric(cut(cra$pH_ave,breaks = n))]
no$not = NrbPal(n)[as.numeric(cut(no$pH_ave,breaks = n))]

plot(dist_filt ~ time, data = data, type = "n", bty = "n", main = "snails paths after start", ylab = "distance from origin (cm)")
#plot(0:44,seq(from=5, to = 10, length.out = 45), type = "n", bty = "n", main = "snails paths after start", ylab = "distance from origin (cm)")
pisind = levels(as.factor(pis$ind))#[which(pis$p == "#FF0000")]
for(i in pisind){
  ind = which(pis$ind == i)
  #filt = pis[ind,][order(pis$time[ind]),]
  lines(pis$dist_filt[ind] ~ pis$time[ind], col = pis$p[ind][1], lwd = 2)
  print(c(i,pis$p[ind][1]))
  #points(i,pis$pH_ave[ind][1], col = p[ind][1], pch = 19)
}

plot(dist_filt ~ time, data = data, type = "n", bty = "n", main = "snails paths after start", ylab = "distance from origin (cm)",xlim = c(5,30))
#plot(0:44,seq(from=5, to = 10, length.out = 45), type = "n", bty = "n", main = "snails paths after start", ylab = "distance from origin (cm)")
craind = levels(as.factor(cra$ind))#[which(pis$p == "#FF0000")]
for(i in craind){
  ind = which(cra$ind == i)
  #filt = pis[ind,][order(pis$time[ind]),]
  lines(cra$dist_filt[ind] ~ cra$time[ind], col = cra$cra[ind][1], lwd = 2, xlim = c(5,30))
  print(c(i,cra$cra[ind][1]))
  #points(i,pis$pH_ave[ind][1], col = p[ind][1], pch = 19)
}

plot(dist_filt ~ time, data = data, type = "n", bty = "n", main = "snails paths after start", ylab = "distance from origin (cm)", xlim = c(5,30))
#plot(0:44,seq(from=5, to = 10, length.out = 45), type = "n", bty = "n", main = "snails paths after start", ylab = "distance from origin (cm)")
noind = levels(as.factor(no$ind))#[which(pis$p == "#FF0000")]
for(i in noind){
  ind = which(no$ind == i)
  #filt = pis[ind,][order(pis$time[ind]),]
  lines(no$dist_filt[ind] ~ no$time[ind], col = no$not[ind][1], lwd = 2, xlim = c(5,30))
  print(c(i,no$not[ind][1]))
  #points(i,pis$pH_ave[ind][1], col = p[ind][1], pch = 19)
}
...
data20 = data[data$dist_filt > 20.0,]
c.agg = aggregate(dist_filt~ind, data, max)
...
###plotting it
par(mfrow=c(1,1))
plot(x = c.both$time, y = c.both$pH_ave, data = c.both, type = "n", bty = "n", main = "time to max distance from origin", ylab = "pH")
points(Cpis$time, Cpis$pH_ave, data = Cpis, col = "purple", pch = 19)
points(Ccra$time, Ccra$pH_ave, data = Ccra, col = "red", pch = 19)
points(Cno$time, Cno$pH_ave, data = Cno, col = "black", pch = 19)
#looking at how long it takes to get to a distance of 20 cm (ie, the corner in most cases)
data20 = data[data$dist_filt > 10.0,]
c.20 = aggregate(time~ind, data20, min)
c.all = merge(c.20, data)
plot(c.all$pH_ave, c.all$time, col = as.factor(c.all$treatment), pch = 19)
#looks like an exponential curve based on pH but not on cue type.

plot(data[data$time < 8,]$dist_filt ~ data[data$time < 8,]$pH_ave, data = data[data$time < 8,], pch = 19, col = as.factor(data[data$time < 8,]$treatment))
...
```{r}
#average coordinates
plot(avY ~ avX, data = data3, bty = "n", type = "n", main = "average coordinates", ylim = c(-60,60), xlim = c(-40,40))
...
#just pisaster. NOT DONE; WILL NOT FUNCTION
sea = data3[data3$Cue == "pisaster",]
Vm <- getInitial(sea$mdist ~ SSmicmen(sea$pH, max(sea$mdist), 6.6), data = sea)[1]
K <- getInitial(mdist ~ SSmicmen(pH, max(mdist), 7), data = sea)[2]
m.m <- nls(mdist ~ SSmicmen(pH, Vm, K), data = sea)
m.m
...
data3$Cue[data3$Cue == 1] = "pisaster"
data3$Cue[data3$Cue == 2] = "crab"
plot(midist ~ pH, data = data3, bty = "n", type = "n", main = "min distance from origin", ylab = "min distance (cm)", xlab = "pH treatment")
...
data3$Cue[data3$Cue == 1] = "pisaster"
data3$Cue[data3$Cue == 2] = "crab"
plot(avedis2 ~ pH, data = data3, bty = "n", type = "n", main = "ave distance from origin", ylab = "distance(cm)", xlab = "pH treatment", ylim = c(0,50))
...
legend('topleft', legend =c('pisaster', 'crab', 'no cue'), 
       box.lty = 0,
       fill = c('purple', 'red', 'black'),  bg= 'white')#adding a figure legend
#plotting a saturing curve fit to the data
abline(lm(avedis2 ~ poly(pH) * Cue, data = data3))
...
plot(wand ~ pH, data = data3, bty = "n", type = "n", main = "wandering", ylim = c(0,5))
...
```
  #total distance? a sum of all of the distance from origin points. 
```{r}
outs = data3[data3$P_out2 >0,]

plot(total ~ pH, data = data3, bty = "n", type = "n", main = "total distance from origin", ylab = "total distance (cm)", xlab = "pH treatment")
points(total ~ pH, data = data3[data3$Cue == "pisaster",], col = "purple", pch = 19)
points(total ~ pH, data = data3[data3$Cue == "crab",], col = "red", pch = 19)
points(total ~ pH, data = outs, col = "black", pch = 19)
points(total ~ pH, data = data3[data3$Cue == "no cue",], col = "grey", pch = 19)
legend('topleft', legend =c('pisaster', 'crab', 'no cue'), 
       box.lty = 0,
       fill = c('purple', 'red', 'black'),  bg= 'white')#adding a figure legend
...
```{r}
#first 5 min
data_start = data[data$time < 5,]
data_start = data_start[data_start$ind != 7 & data_start$ind != 192 & data_start$ind != 193 & data_start$ind != 194 & data_start$ind != 195 & data_start$ind != 39 & data_start$ind != 50 & data_start$ind != 67 & data_start$ind != 148 & data_start$ind != 205 & data_start$ind != 37 & data_start$ind != 79 & data_start$ind != 18 & data_start$ind != 188 & data_start$ind != 138 & data_start$ind != 140 & data_start$ind != 225 & data_start$ind != 128,]
data_start$treatment[data_start$treatment == "0"] = "no cue"
  data_start$treatment[data_start$treatment == "1"] = "pisaster"
  data_start$treatment[data_start$treatment == "2"] = "crab"

with(data_start,plot(dist~pH_ave, col = as.factor(data_start$treatment),pch = 19, main = "dist travel in 5 min"))

LM1 = lmer(dist ~ pH_ave * treatment + (1|Trial_main) + (1|acc_tank), data = data_start)
summary(LM1)
#abline(intercept, slope)
abline(-93.076, 15.4306, col = "purple")
abline(-34.4351, 6.1881,col = "black")
abline(-51.1593,9.5092, col = "red")
...
where do individuals end up?
```{r}
#need to subset so only looking at max time of 30 or 32 for an individual
end = data[data$time == 32 | data$time == 30,]

#filtering out all of the wonky datapoints based on going over raw data notes.
end = end[end$ind != 7 & end$ind != 192 & end$ind != 193 & end$ind != 194 & end$ind != 195 & end$ind != 39 & end$ind != 50 & end$ind != 67 & end$ind != 148 & end$ind != 205 & end$ind != 37 & end$ind != 79 & end$ind != 18 & end$ind != 188 & end$ind != 138 & end$ind != 140 & end$ind != 225 & end$ind != 128,]
...
BOOTSTRAPPING;
trying to bootstrap in_out data to look at 95CI overlap between ambient pH (8) and 7.7pH values-- can use this to calculate the magnitude change from ambient. 
```{r}
#bootstrapping for CI around pH = 8 and pH = 7.7
#This is the final statistical model used for the experiment 
glmer1 = glmer(cbind(Out2, In2) ~ cue * pH + (1|trial)+ (1|accbin)+ (1|ind), data = d1, family = binomial, control=glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000))) 
...
amb_crab = mean(inflecC[inflecC$acclim ==8,]$y) #crab
amb_no = mean(inflecN[inflecN$acclim ==8,]$z) #no cue
amb_pis = mean(inflecP[inflecP$acclim ==8,]$q) #pisaster
test.dataframe=c(amb_crab, amb_no, amb_pis)
...
data.boot = boot(data = d1, statistic = flee, R=100)
data.boot
plot(data.boot, index=1)
boot.ci(data.boot, type='perc', conf=.95, index=1)
boot.ci(data.boot, type='perc', conf=.95, index=2)
