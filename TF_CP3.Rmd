...
```{r}
par(mfrow = c(1,2))
with(d2, plot(P_bot ~ pH, type = "n", xlab = "pH acclimation", ylab = "prop time on wall", bty = "n", main = "Prop wall"))
...
```{r}
par(mfrow = c(2,2))
with(d2, plot(P_bot ~ pH, type = "n", xlab = "pH acclimation", ylab = "prop time in corner", bty = "n", main = "Prop corner"))
...
###model for time spent in corner.
cor = glmer(cbind(Corner, Not_cor) ~ cue * pH + (1|ind) + (1|accbin) + (1|trial), data = d2, family = binomial, control= glmerControl(optimizer="bobyqa", optCtrl=list(maxfun=100000)))
summary(cor)
#arena and trial were negli so dropped them from the model
#AIC 
...
legend(x = 6.5, y = 1.1, legend = c('pisaster', 'crab', 'no cue'), box.lty = 0, fill = c('purple', 'red', 'black'), bg = "white")
```
```{r}
